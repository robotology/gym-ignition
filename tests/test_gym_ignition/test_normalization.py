...
test_matrix = [
    ...
    ([-1., 0, 1, 2], 1 , 1, [-1., 0, 1, 2]),
    # List / List
    ([-1, 0, 2.], [-1, -2, 1], [-1, 4, 3], [-1, -0.3333333, 0]),
]


@pytest.mark.parametrize("input,low, high, output",
                         test_matrix)
def test_normalization(input, low, high, output):
    ...
    assert input == pytest.approx(denormalize(input=normalized_input, low=low, high=high))
